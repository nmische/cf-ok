component extends="mxunit.framework.TestCase" {
	...
	public void function setup(){
		...
		rule = new ok.rules.PastRule(messageProvider);
	}
	
	public void function testRuleNowPasses(){
		var obj = new ok.tests.entities.FooPastTest();
		var md = getMetadata(obj);
		var prop = md.properties[2];
		obj.setNowProp(dateAdd('d',-1,now()));
		var result = rule.isValid(obj, prop, 'ok_past');
		assertTrue(result);
	}
	...
	public void function testRuleReturnsMessage(){
		...
		var prop = md.properties[2];
		obj.setNowProp(dateAdd('d',1,now()));
		var msg = rule.getMessage(obj, prop, 'ok_past');
		assertEquals('Now prop is not before #dateFormat(now())#.', msg);
	}
